...
> **~1.5 B/point 가정의 실측 검증 (M1, 2026-07-24)**: XOR 인코딩 압축률을 메트릭 유형별로 측정한 결과 — 온도(정수 계단) 0.27 · 사용률(정수 %) 2.37 · 메모리(느린 계단) 0.19 · 카운터(단조증가) 2.64 B/point. 실제 하드웨어 메트릭은 대부분 정수 계단형이라 1.5 가정을 **실측이 뒷받침**한다(대체로 더 좋다). 단 **소수점이 계속 요동하는 float**(load1 0.01 단위 등)은 유효비트가 가수부 전체에 퍼져 ~6.7 B/point 로 나빠진다 — 그런 시리즈가 많으면 위 산정이 최대 4배까지 커질 수 있으므로, 그 경우 `retention.rollup` 을 낮추거나 §8.4 선택적 수집으로 대응한다.
