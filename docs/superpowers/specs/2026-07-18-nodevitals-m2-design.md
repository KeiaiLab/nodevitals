...
| mem | /proc/meminfo | `NewFS(root).Meminfo()` | mem_used_bytes, mem_available_bytes, swap_* |
| net | /proc/net/dev | `NewFS(root).NetDev()` | net_rx/tx_bytes, net_*_errors |
| disk | /proc/diskstats | `blockdevice.NewFS(proc,sys).ProcDiskstats()` | disk_io_bytes, disk_io_ops |
| **hwmon** | /sys/class/hwmon | **손수 파싱** (procfs 미지원) | temp_celsius, fan_rpm, in_volts |
